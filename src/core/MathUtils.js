...
export class MathUtils {
  ...
  static closestPointOnLine(a, b, p) {
    ...
    const t = ((p[0] - a[0]) * (b[0] - a[0]) + (p[1] - a[1]) * (b[1] - a[1])) / distSqr;
  ...
  static getDelaunayTriangulation(vertices) {
    if (!vertices || vertices.length < 3) {
      throw new Error(`Cannot get delaunay triangulation for points ${vertices}. Input must contain at least three points.`);
    }
    ...
    const superIndices = [vertices.length, vertices.length + 1, vertices.length + 2];

    const vertsWithSuper = [...vertices, [midX - 20 * dMax, midY - dMax], [midX, midY + 20 * dMax], [midX + 20 * dMax, midY - dMax]];

    const superSortedIndices = MathUtils.sortPointsCCW(superIndices, vertsWithSuper);
    ...
    vertsWithSuper.forEach((newVert, newIndex) => {
      ...
      boundingPoly.forEach((edge) => {
        const sortedIndices = MathUtils.sortPointsCCW([edge[0], edge[1], newIndex], vertsWithSuper);
  ...
  /**
   * Determines if a given 2D point is within the cicrumcircle defined by three 2D points. The triangle points must be in counter-clockwise order a -> b -> c.
   *
  ...
  static isPointInTriangle(a, b, c, p) {
    ...
    const aVal = ((b[1] - c[1]) * (p[0] - c[0]) + (c[0] - b[0]) * (p[1] - c[1])) / denom;
    const bVal = ((c[1] - a[1]) * (p[0] - c[0]) + (a[0] - c[0]) * (p[1] - c[1])) / denom;
  ...
  /**
   * Gets the sorted indices of a given set of 2D points in counter-clockwise order.
   *
  ...
  static sortPointsCCW(indices, vertices) {
    ...
    indices.sort((a, b) => {
      const bVal = Math.atan2(vertices[b][1] - centroid[1], vertices[b][0] - centroid[0]);
      const aVal = Math.atan2(vertices[a][1] - centroid[1], vertices[a][0] - centroid[0]);
  ...
  static getVectorMagnitude(vector) {
    if (!(vector instanceof Array)) {
      throw new Error(`Cannot get magnitude for vector ${vector}. Input must be an Array numbers.`);
    }

    const reducer = (accumulator, currentValue) => {
      if (typeof currentValue !== 'number') {
        throw new Error(`Cannot get magnitude for vector ${vector}. All items in the input Array must be numbers.`);
      }
  ...
  static rotateVector(vector3, matrix3) {
    if (!(vector3 instanceof Array) || vector3.length !== 3 || !vector3.every((v) => typeof v === 'number')) {
      throw new Error(`Cannot rotate vector ${vector3} by rotation matrix ${matrix3}. Input vector must be an array of 3 numbers.`);
    }

    if (!(matrix3 instanceof Array) || matrix3.length !== 9 || !matrix3.every((v) => typeof v === 'number')) {
      throw new Error(`Cannot rotate vector ${vector3} by rotation matrix ${matrix3}. Input matrix3 must be an array of 9 numbers.`);
    }

    const x = matrix3[0] * vector3[0] + matrix3[3] * vector3[1] + matrix3[6] * vector3[2];
    const y = matrix3[1] * vector3[0] + matrix3[4] * vector3[1] + matrix3[7] * vector3[2];
    const z = matrix3[2] * vector3[0] + matrix3[5] * vector3[1] + matrix3[8] * vector3[2];
  ...
  static getRotationMatrix(matrix4) {
    if (!(matrix4 instanceof Array) || matrix4.length !== 16 || !matrix4.every((v) => typeof v === 'number')) {
      throw new Error(`Cannot convert matrix ${matrix4} to a rotation matrix. Input matrix must be an array of 16 numbers.`);
    }
   ...
   */
  static dampValue(currentValue, targetValue, valueStore = [0, 0], deltaTime = 1e-7, smoothTime = 0.3, maxSpeed = 1e7) {
