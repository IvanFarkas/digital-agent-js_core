...
describeHostEnvironment('Messenger', () => {
  ...
  describe('listenTo', () => {
    ...
    // Note: TypeScript compiler takes care of this
    // it('does not throw an error if the message is not a string', () => {
    //   const listener = () => {};

    //   expect(messenger.listenTo.bind(messenger, 5, listener)).not.toThrowError();

    //   expect(messenger.listenTo.bind(messenger, Messenger, listener)).not.toThrowError();
    // });
  });

  describe('stopListening', () => {
    it('can stop a specific listener from being executed when a message is received', () => {
      const listener1 = jest.fn().mockName('listener1');
      const listener2 = jest.fn().mockName('listener2');
    ...
    it('can stop all listeners from being executed when a message is received', () => {
      const listener1 = jest.fn().mockName('listener1');
      const listener2 = jest.fn().mockName('listener2');
      const listener3 = jest.fn().mockName('listener3');
    ...
    //   expect(messenger.stopListening.bind(messenger, messenger)).not.toThrowError();
    // });

    it('fails gracefully if the callback is not a function', () => {
      expect(messenger.stopListening.bind(messenger, 'test', null)).not.toThrowError();
    });
  });
  ...
  describe('stopListeningToAll', () => {
    it('can stop all listeners from being executed when any message is received', () => {
      ...
      const listener2 = jest.fn().mockName('listener2');
      const listener3 = jest.fn().mockName('listener3');
      const listener4 = jest.fn().mockName('listener4');
  ...
  describe('emit', () => {
    ...
    it('does not throw errors if there are no listeners for a message', () => {
      expect(messenger.emit.bind(messenger, 'messageWithNoListeners')).not.toThrowError();
    });
