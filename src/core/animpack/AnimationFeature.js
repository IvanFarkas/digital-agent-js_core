...
export class AnimationFeature extends AbstractHostFeature {
  ...
  /**
   * Re-evaluate internal weight values of layers starting from the top of the stack. Override layers' weights affect the values of all layers lower in the stack.
   *
   * @private
   */
   ...
   * @param {string} [options.blendMode=LayerBlendModes[DefaultLayerBlendMode]] - Type of blending the animation should use.
   * @param {Array.<Object>} [options.blendStateOptions] - Array of options used to create the blend states for this container.
   * @param {number[]} [options.blendThresholds] - Array of numbers used to set the thresholds for each blend state in this container.
   * @param {boolean[]} [options.blendMatchPhases=[]] - Optional array of booleans used to set whether or not each blend state in this container will match phases.
   *
   ...
   * @param {string} [options.blendMode=LayerBlendModes[DefaultLayerBlendMode]] - Type of blending the animation should use.
   * @param {Array.<Object>} [options.blendStateOptions] - Array of options used to create the blend states for this container.
   * @param {number[][]} [options.blendThresholds] - Array of Array of numbers used to set the thresholds for each blend state in this container.
   * @param {boolean[]} [options.blendMatchPhases=[]] - Optional array of booleans used to set whether or not each blend state in this container will match phases.
   *
  ...
  _validateNewAnimation(layerName, animationName) {
    ...
    if (layer === undefined) {
      throw new Error(`Cannot add animation to layer ${layerName} from host ${this._host.id}. No layer exists with this name.`);
    }
    ...
    if (name !== animationName) {
      console.warn(`Animation name ${animationName} is not unique for layer ${layer.name}. Animation will be renamed to ${name}.`);
    }
  ...
  addLayer(name = 'NewLayer', options = {}, index) {
    ...
    if (index === undefined || index === -1) {
      ...
      if (layerIndex === undefined) {
        ...
        console.warn(`Index ${index} is invalid for host ${this._host.id}. New layer will be added at the closest valid index: ${layerIndex}.`);
      }
    }
  ...
  removeLayer(name) {
    ...
    if (layer === undefined) {
      console.warn(`Did not remove layer ${name} from host ${this._host.id}. No layer exists with this name.`);
      return false;
    }
  ...
  moveLayer(name, index) {
    ...
    if (layer === undefined) {
      throw new Error(`Cannot move layer ${name} from host ${this._host.id}. No layer exists with this name.`);
    }
    ...
    if (layerIndex === undefined) {
      throw new Error(`Cannot move layer ${name} from host ${this._host.id} to index ${index}. Index must be in the 0 - ${lastIndex} range.`);
    }
  ...
  renameLayer(currentName, newName) {
    ...
    if (layer === undefined) {
      throw new Error(`Cannot rename layer ${currentName} from host ${this._host.id}. No layer exists with this name.`);
    }
    ...
    if (name !== newName) {
      console.warn(`Layer name ${newName} is not unique. Layer will be renamed to ${name}.`);
    }
  ...
  getLayerWeight(name) {
    ...
    if (layer === undefined) {
      throw new Error(`Cannot get weight on layer ${name} from host ${this._host.id}. No layer exists with this name.`);
    }
  ...
  getAnimationBlendNames(layerName, animationName) {
    ...
    if (layer === undefined) {
      throw new Error(`Cannot get blend names on layer ${layerName} from host ${this._host.id}. No layer exists with this name.`);
    }
   ...
   */
  setAnimationBlendWeight(layerName, animationName, blendName, weight, seconds, easingFn) {
    const layer = this._layerMap[layerName];
  ...
  getAnimationBlendWeight(layerName, animationName, blendName) {
    ...
    if (layer === undefined) {
      throw new Error(`Cannot get blend weight on layer ${layerName} from host ${this._host.id}. No layer exists with this name.`);
    }
  ...
  pauseLayerWeight(name) {
    ...
    if (layer === undefined) {
      throw new Error(`Cannot pause weight interpolation on layer ${name} from host ${this._host.id}. No layer exists with this name.`);
    }
  ...
  pauseLayer(name) {
    ...
    if (layer === undefined) {
      throw new Error(`Cannot pause layer ${name} from host ${this._host.id}. No layer exists with this name.`);
    }
  ...
  getTransitioning(layerName) {
    ...
    if (layer === undefined) {
      throw new Error(`Get transitioning on layer ${layerName} from host ${this._host.id}. No layer exists with this name.`);
    }
  ...
  getAnimations(layerName) {
    ...
    if (layer === undefined) {
      throw new Error(`Get animations on layer ${layerName} from host ${this._host.id}. No layer exists with this name.`);
    }
  ...
  getCurrentAnimation(layerName) {
    ...
    if (layer === undefined) {
      throw new Error(`Get current animation on layer ${layerName} from host ${this._host.id}. No layer exists with this name.`);
    }
  ...
  getPaused(layerName) {
    ...
    if (layer === undefined) {
      throw new Error(`Get paused on layer ${layerName} from host ${this._host.id}. No layer exists with this name.`);
    }
  ...
  getAnimationType(layerName, animationName) {
    ...
    if (layer === undefined) {
      throw new Error(`Get animation type on layer ${layerName} from host ${this._host.id}. No layer exists with this name.`);
    }
  ...
  addAnimation(layerName, animationName, animationType = AnimationTypes.single, options = {}) {
    ...
    if (!Object.values(AnimationTypes).includes(animationType)) {
      throw new Error(`Cannot add animation ${animationName} to layer ${layerName} on host ${this._host.id}. Invalid animation type.`);
    }
  ...
  removeAnimation(layerName, animationName) {
    ...
    if (layer === undefined) {
      throw new Error(`Cannot remove animation from layer ${layerName} from host ${this._host.id}. No layer exists with this name.`);
    }
  ...
  renameAnimation(layerName, currentAnimationName, newAnimationName) {
    ...
    if (layer === undefined) {
      throw new Error(`Cannot rename animation ${currentAnimationName} on layer ${layerName} for host ${this._host.id}. No layer exists with this name.`);
    }
  ...
  pauseAnimation(name) {
    ...
    if (layer === undefined) {
      console.warn(`Did not pause animation on layer ${name} for host ${this._host.id}. No layer exists with this name.`);
      return false;
    }
  ...
  stopAnimation(name) {
    ...
    if (layer === undefined) {
      console.warn(`Did not stop animation on layer ${name} for host ${this._host.id}. No layer exists with this name.`);
      return false;
    }
