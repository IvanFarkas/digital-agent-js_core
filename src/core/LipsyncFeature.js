 ...
 * @property {any} viseme - Viseme
 * @property {any} visemeMap - Viseme Map
 * @property {boolean} isVisemeActive - Is Viseme Active.
 * @property {number} decayRate - An object describing the 0-1 factor viseme weight will decay if the viseme duration is longer than the blendTime and the number of seconds it would take to decay by that factor.
 * @property {number} blendTime - Default amount of time to enable and disable the talking idle layers
 * @property {number} overrideWeight - Override Weight.
 * @property {number} easingFn - Default easing function to use when manipulating viseme freeBlend weights.
 * @property {boolean} isActive - Is Active.
 * @property {Function=} registerLayer - Register Layer
 * @property {Function=} enable - Enable
 * @property {Function=} disable - Disable
 */
export class LipsyncFeature extends AbstractHostFeature.mix(TextToSpeechFeatureDependentInterface.Mixin, ManagedAnimationLayerInterface.Mixin) {
   ...
   * @param {Array.<Object>} [visemeOptions.layers=[]] - An array of layer options objects to register as viseme layers.
   ...
   * @param {Array.<Object>} [talkingOptions.layers=[]] - An array of layer options objects to register as talking layers.
  ...
  constructor(host, /* visemeOptions= */ { blendTime: visemeBlendTime = 0.15, decayRate: { amount = 0.5, seconds = 0.5 } = {}, easingFn: visemeEasingFn = Quadratic.InOut, layers: visemeLayers = [] }, /* talkingOptions= */ { blendTime: talkingBlendTime = 0.75, easingFn: talkingEasingFn = Quadratic.InOut, layers: talkingLayers = [] }, visemeLeadTime = 0.067) {
    ...
    // TODO: Test how input params are passed and used.
  ...
  _registerVisemeAnimation(layerName, animationName) {
    ...
    if (animation.isActive) {
      const animationType = this._host.AnimationFeature.getAnimationType(layerName, animationName);
      ...
      if (animationType !== 'freeBlend') {
        ...
        const weightNames = animationBlendNames;
        const { visemeMap } = animation;
  ...
  async _onViseme({ mark }) {
    Object.entries(this._visemeLayers).forEach(([layerName, animName]) => {
      ...
      const { isActive, visemeMap } = this._managedLayers[layerName].animations[animName];
      ...
      const visemeNames = this._host.AnimationFeature.getAnimationBlendNames(layerName, animName);
      ...
      // Find the peak weight for the viseme and the amount of time it should take to reach it
      let weight = 1;
      const duration = mark.duration / 1000;
      if (duration < blendTime) {
        const lerpFactor = blendTime > 0 ? duration / blendTime : 1;
        weight = overrideWeight !== undefined ? overrideWeight : MathUtils.lerp(0, 1, lerpFactor);
      }
      ...
      if (holdTime < 0) {
        ...
        this._animateSimpleViseme(layerName, animName, viseme, weight, blendInTime, blendTime, easingFn);
      } else {
        const lerpFactor = decayRate.seconds > 0 ? holdTime / decayRate.seconds : 1;
        // Perform in -> hold -> out animation
        const decayWeight = MathUtils.lerp(weight, weight * decayRate.amount, Math.min(1, lerpFactor));
        this._animateHeldViseme(layerName, animName, viseme, weight, decayWeight, blendInTime, holdTime, blendTime, easingFn);
      }
    });
  }
   ...
   */
  _animateSimpleViseme(layerName, animName, visemeName, peakWeight, blendInTime, blendOutTime, easingFn) {
    // Animate towards the peak value
    const weightPromise = this._host.AnimationFeature.setAnimationBlendWeight(layerName, animName, visemeName, peakWeight, blendInTime, easingFn);
   ...
   * @param {Function} easingFn - Easing function to use during animation.
   */
  async _animateHeldViseme(layerName, animName, visemeName, peakWeight, decayWeight, blendInTime, holdTime, blendOutTime, easingFn) {
    // Animate towards the peak value
    const startPromise = this._host.AnimationFeature.setAnimationBlendWeight(layerName, animName, visemeName, peakWeight, blendInTime, easingFn);

    await startPromise;

    if (!startPromise.canceled) {
      // Animate towards the decay value if there was no weight interruption
      const holdPromise = this._host.AnimationFeature.setAnimationBlendWeight(layerName, animName, visemeName, decayWeight, holdTime, easingFn);
  ...
  /**
   * Gets the amount of time in seconds to negatively offset speechmark emission by.
   *
   * @type {number}
   */
  get visemeLeadTime() {
  ...
  /**
   * Sets the amount of time in seconds to negatively offset speechmark emission by.
   *
   * @type {number}
   */
   ...
   */
  registerVisemeLayer(layerName, { animation = 'visemes', decayRate = { amount: 0.5, seconds: 0.5 }, blendTime = LipsyncFeature.DEFAULT_LAYER_OPTIONS.blendTime, easingFn, visemeMap = DefaultVisemeMap } = {}) {
   ...
   */
  registerTalkingLayer(layerName, { animation = 'stand_talk', blendTime = LipsyncFeature.DEFAULT_LAYER_OPTIONS.blendTime, easingFn } = {}) {
    // Register the layer and animation
    this.registerLayer(layerName, {
      blendTime,
      easingFn,
