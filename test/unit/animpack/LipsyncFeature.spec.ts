...
describeHostEnvironment('LipsyncFeature', () => {
  let lipsyncFeature: LipsyncFeature;
  ...
  beforeEach(() => {
    ...
    mockAnimationFeature.getAnimationType = jest.fn(() => 'freeBlend');
    mockAnimationFeature.getAnimationBlendNames = jest.fn(() => Object.keys(DefaultVisemeMap));
    mockAnimationFeature.resumeAnimation = jest.fn(() => Deferred.resolve());
    mockAnimationFeature.setAnimationBlendWeight = jest.fn(() => Deferred.resolve());
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
  ...
  describe('_registerVisemeAnimation', () => {
    ...
    it('should log a warning and deactivate the animation if it is not a freeBlend state', () => {
      ...
      expect(lipsyncFeature._managedLayers.Lipsync.animations.visemes.isActive).toBeTrue();

      lipsyncFeature._registerVisemeAnimation('Lipsync', 'visemes');

      expect(onWarn).toHaveBeenCalledTimes(1);
      onWarn.mockRestore();
      expect(lipsyncFeature._managedLayers.Lipsync.animations.visemes.isActive).toBeFalse();
    });

    it('should set isActive to true on the visemeMap for any viseme whose name matches a blendWeight name', () => {
      Object.values(lipsyncFeature._managedLayers.Lipsync.animations.visemes.visemeMap).forEach((visemeOptions: any) => {
  ...
  describe('_onLayerAdded', () => {
    let onRegisterVisemeAnimation: any;

    beforeEach(() => {
      onRegisterVisemeAnimation = jest.spyOn(lipsyncFeature, '_registerVisemeAnimation').mockImplementation(() => {});
    });
    ...
    it('should execute _registerVisemeAnimation if the layer name is defined in _visemeLayers', () => {
      ...
      expect(onRegisterVisemeAnimation).toHaveBeenCalledWith('Lipsync', 'visemes');
      onRegisterVisemeAnimation.mockRestore();
    });
  });
  ...
  describe('_onPlay', () => {
    ...
    beforeEach(() => {
      onEnable = jest.spyOn(lipsyncFeature, 'enable');
      lipsyncFeature._managedLayers = {
        Lipsync: {
          isActive: true,
          animations: {
    ...
    it('should execute enable', () => {
      ...
      onEnable.mockRestore();
    });

    it('should execute resumeAnimation on any active managed animations', () => {
      lipsyncFeature._onPlay();

      expect(mockAnimationFeature.resumeAnimation).toHaveBeenCalledWith('Lipsync', 'visemes');

      expect(mockAnimationFeature.resumeAnimation).not.toHaveBeenCalledWith('Talk', 'stand_talk');
      onEnable.mockRestore();
    });
  });
  ...
  describe('_onPause', () => {
    ...
    beforeEach(() => {
      onStop = jest.spyOn(lipsyncFeature, '_onStop');
    });
  ...
  describe('_onResume', () => {
    ...
    beforeEach(() => {
      onPlay = jest.spyOn(lipsyncFeature, '_onPlay');
    });
  ...
  describe('_onStop', () => {
    ...
    beforeEach(() => {
      onDisable = jest.spyOn(lipsyncFeature, 'disable');
    ...
    it('should execute disable', () => {
      ...
      onDisable.mockRestore();
    });

    it('should execute pauseAnimation on any active managed animations', () => {
      lipsyncFeature._onStop();

      expect(mockAnimationFeature.pauseAnimation).toHaveBeenCalledWith('Lipsync', 'visemes');

      expect(mockAnimationFeature.pauseAnimation).not.toHaveBeenCalledWith('Talk', 'stand_talk');
  ...
  describe('_onViseme', () => {
    let onAnimateSimpleViseme: any;
    let onAnimateHeldViseme: any;

    beforeEach(() => {
      onAnimateSimpleViseme = jest.spyOn(lipsyncFeature, '_animateSimpleViseme');
      onAnimateHeldViseme = jest.spyOn(lipsyncFeature, '_animateHeldViseme');

      const visemeMap = {...DefaultVisemeMap};

      Object.values(visemeMap).forEach((visemeOptions: any) => {
    ...
    it('should not execute _animateSimpleViseme or _animateHeldViseme if there are no active viseme animations', () => {
      ...
      onAnimateSimpleViseme.mockRestore();
      onAnimateHeldViseme.mockRestore();
    });
    ...
    it("should execute _animateSimpleViseme if mark.duration is less than the viseme's blendTime", () => {
      const {visemeMap} = lipsyncFeature._managedLayers.Lipsync.animations.visemes;
  ...
  describe('_animateSimpleViseme', () => {
    it('should execute AnimationFeature.setAnimationBlendWeight once if setAnimationBlendWeight is called again externally before it finishes', () => {
      mockAnimationFeature.setAnimationBlendWeight = jest.fn(() => {
      ...
      expect(mockAnimationFeature.setAnimationBlendWeight).toHaveBeenCalledTimes(1);
    ...
    it('should execute AnimationFeature.setAnimationBlendWeight twice if setAnimationBlendWeight is not called again externally before it finishes', async () => {
      ...
      expect(mockAnimationFeature.setAnimationBlendWeight).toHaveBeenCalledTimes(2);
    });
  });
  ...
  describe('_animateHeldViseme', () => {
    ...
    it('should execute AnimationFeature.setAnimationBlendWeight three times if setAnimationBlendWeight is not called again externally before it finishes', async () => {
      ...
      expect(mockAnimationFeature.setAnimationBlendWeight).toHaveBeenCalledTimes(3);
    });
  });

  describe('registerVisemeLayer', () => {
    it('should execute registerLayer before _registerVisemeAnimation', () => {
      const onRegisterLayer = jest.spyOn(lipsyncFeature, 'registerLayer');
      const onRegisterVisemeAnimation = jest.spyOn(lipsyncFeature, '_registerVisemeAnimation');
      lipsyncFeature.registerVisemeLayer('Lipsync', {animations: 'viseme'});

      // TODO: Fix P1. Find how to use toHaveBeenCalledBefore() properly. Does not like arg.
      // expect(onRegisterLayer).toHaveBeenCalledBefore(onRegisterVisemeAnimation);

      const onRegisterLayerOrder = onRegisterLayer.mock.invocationCallOrder[0];
      const onRegisterVisemeAnimationOrder = onRegisterVisemeAnimation.mock.invocationCallOrder[0];

      expect(onRegisterLayerOrder).toBeLessThan(onRegisterVisemeAnimationOrder);
