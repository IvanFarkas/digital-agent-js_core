...
export class TextToSpeechUtils {
  ...
  static autoGenerateSSMLMarks(text, map, randomMarks = []) {
    if (typeof map !== 'object' || map === null) {
      throw new Error(`Cannot generate SSML marks for text "${text}" because map is not an object.`);
    }
    ...
    existingTags.forEach((existingTag) => {
      ...
      if (substr !== '') {
        ...
        chunks.push(this._insertMarks(substr, internalMap, duplicateMarkToCheck));
  ...
  static addMarksToUnmarkedSentences(text, marks) {
    ...
    const randomMarkedText = this._insertRandomMarksAt(text, targetIndices, marks);
   ...
   *
   * @param {string} text - Input string.
   * @param {number[]} [indices = []] - An array of indices in the text input where random marks should be inserted.
  ...
  static _insertRandomMarksAt(text, indices = [], marks = []) {
    ...
    indices.forEach((index) => {
      const randomMark = `<mark name='${marks[Utils.getRandomInt(0, marks.length)]}'/>`;
   ...
   *
   * @returns {number[]} - Array of end of sentence indices.
   */
  ...
  static _processInputMap(map) {
    ...
    Object.entries(map).forEach(([key, value]) => {
      if (!Array.isArray(value)) {
        throw new Error(`Cannot generate SSML marks from map "${map}" because value for key '${key}' is not an array.`);
      }
