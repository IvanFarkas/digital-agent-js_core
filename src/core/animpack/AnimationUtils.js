...
export class AnimationUtils {
   ...
   */
  static interpolateProperty(propertyOwner, propertyName, targetValue, { seconds = 0, easingFn, onFinish, onProgress, onCancel, onError } = {}) {
    // Make sure property is an object
    if (!(propertyOwner instanceof Object)) {
      const e = new Error(`Cannot interpolate property ${propertyName}. Property owner must be an object.`);
    ...
    if (Number.isNaN(Number(propertyOwner[propertyName]))) {
      const e = new Error(`Cannot interpolate property ${propertyName}. Property must be numeric.`);
    ...
    if (Number.isNaN(Number(targetValue))) {
      const e = new Error(`Cannot interpolate property ${propertyName} to value ${targetValue}. Target value must be numeric.`);
      ...
      onProgress: (progress) => {
        if (propertyOwner[propertyName] !== targetValue) {
          ...
          if (typeof easeFactor !== 'number') {
            const e = new Error(`Invalid property interpolation easingFn. EasingFn must return a number.`);
            interpolator.reject(e);
            return;
          }

          // Update the value
          propertyOwner[propertyName] = MathUtils.lerp(startValue, targetValue, easeFactor);
        }
