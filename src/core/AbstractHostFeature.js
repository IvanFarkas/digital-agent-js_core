 ...
 * @property {string} [EVENTS.update=onUpdate] - Message that is emitted after each call to [update]{@link AbstractHostFeature#update}.
...
export class AbstractHostFeature {
  ...
  /**
   * Applies a sequence of mixin class factory functions to this class and returns the result. Each function is expected to return a class that extends the class it was given. The functions are applied in the order that parameters are given, meaning that the first factory will extend this base class.
   *
   * @param {...Function} mixinClassFactories Class factory functions that will be applied.
   *
   * @returns {Class} A class that is the result of applying the factory functions. The resulting class will always inherit from AbstractHostFeature.
   */
