...
export class AbstractState {
   ...
   */
  constructor(options = {}) {
    this._weight = options.weight !== undefined ? MathUtils.clamp(options.weight, 0, 1) : 0;
  ...
  setWeight(weight, seconds = 0, easingFn) {
    ...
    this._promises.weight = AnimationUtils.interpolateProperty(this, 'weight', weight, { seconds, easingFn });
  ...
  play(onFinish, onError, onCancel) {
    ...
    this._promises.finish = Deferred.all([this._promises.play, this._promises.weight]);
  ...
  resume(onFinish, onError, onCancel) {
    ...
    if (!this._promises.play.pending) {
      ...
      this._promises.play = new Deferred(undefined, this._playCallbacks.onFinish, this._playCallbacks.onError, this._playCallbacks.onCancel);
      this._promises.finish = Deferred.all([this._promises.play, this._promises.weight]);
    }
