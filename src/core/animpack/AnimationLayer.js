...
export class AnimationLayer extends AnimationPlayerInterface.Mixin(StateContainerInterface.Mixin()) {
   ...
   * @param {LayerBlendModes} [options.blendMode=DefaultLayerBlendMode] - Type of blending to use for all states controlled by the layer.
  ...
  constructor(options = {}) {
    ...
    this._blendMode = Array.from(Object.values(LayerBlendModes)).includes(options.blendMode) ? options.blendMode : DefaultLayerBlendMode;
    this._promises = { weight: Deferred.resolve() };
  ...
  /**
   * Gets the amount of influence the layer's current animation has over the result for the host.
   *
   * @type {number}
   */
  set weight(weight) {
  ...
  /**
   * Sets the amount of influence the layer's current animation has over the result for the host.
   *
   * @type {number}
   */
  get weight() {
  ...
  setWeight(weight, seconds = 0, easingFn) {
    ...
    this._promises.weight = AnimationUtils.interpolateProperty(this, 'weight', weight, {
      seconds,
  ...
  getAnimationBlendNames(animationName) {
    ...
    if (state === undefined) {
      throw new Error(`Cannot get blend names of animation ${animationName} on layer ${this.name}. No animation exists with this name.`);
    }
    ...
    throw new Error(`Cannot get blend names of animation ${animationName} on layer ${this.name}. Animation is not an instance of AbstractBlendState.`);
  }
   ...
   */
  setAnimationBlendWeight(animationName, blendName, weight, seconds = 0, easingFn) {
    const state = this.getState(animationName);

    if (state === undefined) {
      throw new Error(`Cannot set blend weight of animation ${animationName} on layer ${this.name}. No animation exists with this name.`);
    }
    ...
    throw new Error(`Cannot set blend weight of animation ${animationName} on layer ${this.name}. Animation is not an instance of AbstractBlendState.`);
  }
  ...
  getAnimationBlendWeight(animationName, blendName) {
    ...
    if (state === undefined) {
      throw new Error(`Cannot get blend weight of animation ${animationName} on layer ${this.name}. No animation exists with this name.`);
    }
    ...
    throw new Error(`Cannot get blend weight of animation ${animationName} on layer ${this.name}. Animation is not an instance of AbstractBlendState.`);
  }
