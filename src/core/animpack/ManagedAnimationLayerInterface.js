    ...
    const ManagedAnimationLayerMixin = class extends ParentClass {
      ...
      _onLayerRemoved({ name }) {
        ...
        if (this._managedLayers[name] !== undefined) {
          ...
          Object.keys(this._managedLayers[name].animations).forEach((animName) => {
      ...
      registerLayer(name, options = {}) {
        ...
        layerOptions.isActive = this._host.AnimationFeature !== undefined && this._host.AnimationFeature.layers.includes(name);
      ...
      registerAnimation(layerName, animationName, options = {}) {
        ...
        const animOptions = this._managedLayers[layerName].animations[animationName] || {};
      ...
      setLayerWeights(nameFilter = () => true, weight, seconds, easingFn) {
        ...
        layerNames.forEach((name) => {
          ...
          if (layerOptions.isActive) {
            this._host.AnimationFeature.setLayerWeight(name, weight, seconds !== undefined ? seconds : layerOptions.blendTime, easingFn || layerOptions.easingFn);
          }
        });
      }
