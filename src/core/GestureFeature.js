 ...
 * @property {Function=} setLayerWeights - Set Layer Weights callback
...
export class GestureFeature extends AbstractHostFeature.mix(SSMLSpeechmarkInterface.Mixin, ManagedAnimationLayerInterface.Mixin) {
   ...
   * @param {Object} [options.layers={}] - An object that maps layer names to layer options. These are the layers that will be registered as tracked gesture layers. See {@link ManagedAnimationLayerInterface#registerLayer} for more information on expected format for each layer options object.
   */
  constructor(host, /* options= */ { holdTime = 3, minimumInterval = 1, layers = {} }) {
    super(host);
  ...
  _getHoldTime(layer, animation) {
    const layerHoldTime = layer.holdTime !== undefined ? layer.holdTime : this.holdTime;
  ...
  _getMinimumInterval(layer, animation) {
    const layerMinimumInterval = layer.minimumInterval !== undefined ? layer.minimumInterval : this.minimumInterval;
   ...
   */
  _onNext(layerName, animationName, canAdvance, isQueueEnd) /* {layerName, animationName, canAdvance, isQueueEnd} */ {
   ...
   */
  _onStop(layerName, animationName) /* {layerName, animationName} */ {
   ...
   * @param {string} animationName - Name of the animation that was stopped.
   */
  _onAnimationAdded(layerName, animationName) /* {layerName, animationName} */ {
    const layer = this._managedLayers[layerName];
    ...
    if (this._managedLayers[name].isActive) {
      this._host.AnimationFeature.getAnimations(name).forEach((anim) => {
        ...
        if (this._managedLayers[name].animations[anim] === undefined) {
          this.registerAnimation(name, anim, this._managedLayers[name].animations[anim]);
        }
      });
    }
  }
  ...
  createGestureMap() {
    ...
    Object.entries(this._managedLayers).forEach(([layerName, { animations }]) => {
      Object.entries(animations).forEach(([animationName, { holdTime, minimumInterval, words, transitionTime }]) => {
  ...
  createGenericGestureArray(layers) {
    ...
    layers.forEach((layerName) => {
      ...
      Object.entries(layer.animations).forEach(([animationName, { holdTime, minimumInterval, words, transitionTime }]) => {
   ...
   */
  playGesture(layerName, animationName, { holdTime, minimumInterval, transitionTime, force = false } = {}) {
    ...
    if (!force && layer.playTimer !== null && layer.playTimer < minimumInterval) {
      console.warn(`Skipping gesture ${animationName} on layer ${layerName} for host ${this._host.id}. Minimum interval ${minimumInterval} has not been met.`);
