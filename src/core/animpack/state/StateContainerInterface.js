  ...
  static Mixin(BaseClass = class { }) {
    ...
    const StateContainerMixin = class extends BaseClass {
      ...
      addState(state, debug = false) {
        ...
        const uniqueName = Utils.getUniqueName(state.name, [...this._states.keys()]);

        if (state.name !== uniqueName) {
          console.warn(`Animation name ${state.name} is not unique for state ${this.name}. New animation will be added with name ${uniqueName}.`);
      ...
      removeState(name) {
        ...
        if (!this._states || !this._states.has(name)) {
          console.warn(`Did not remove animation ${name} from state ${this.name}. No animation exists with this name.`);
          return false;
        }
      ...
      renameState(currentName, newName) {
        ...
        if (!this._states || !this._states.has(currentName)) {
          throw new Error(`Cannot rename animation ${currentName} in ${this.name}. No animation exists with this name.`);
        }
        ...
        if (newName !== uniqueName) {
          console.warn(`Animation name ${newName} is not unique in state ${this.name}. Animation will be renamed to ${uniqueName}.`);
          newName = uniqueName;
        }
