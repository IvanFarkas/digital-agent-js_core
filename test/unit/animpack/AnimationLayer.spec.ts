...
import {AnimationLayer, LayerBlendMode} from '@core/animpack/AnimationLayer';
...
describe('AnimationLayer', () => {
  ...
  describe('blendMode', () => {
    it('should return a valid LayerBlendMode', () => {
      expect(Object.values(LayerBlendMode)).toContain(layer.blendMode);
    });
  });
  ...
  describe('weightPending', () => {
    ...
    it('should return false if the weight promise has been rejected', () => {
      ...
      layer._promises.weight.catch((e: any) => {});
  ...
  describe('pause', () => {
    it('should execute pauseAnimation', () => {
      const onPauseAnimation = jest.spyOn(layer, 'pauseAnimation');
      layer.pause();

      expect(onPauseAnimation).toHaveBeenCalledTimes(1);
      onPauseAnimation.mockRestore();
    });

    it('should prevent update from executing the weight promise', () => {
      layer._promises.weight = new Deferred();
      let onExecute = jest.spyOn(layer._promises.weight, 'execute');
      layer.update(200);

      expect(onExecute).toHaveBeenCalledWith(200);

      layer._promises.weight = new Deferred();
      onExecute = jest.spyOn(layer._promises.weight, 'execute');
      layer.pause();
      layer.update(200);
  ...
  describe('resume', () => {
    it('should execute resumeAnimation if there is a current state', () => {
      const onResumeAnimation = jest.spyOn(layer, 'resumeAnimation');
      ...
      onResumeAnimation.mockRestore();
    });

    it('should allow update to execute the weight promise', () => {
      const onExecute = jest.spyOn(layer._promises.weight, 'execute');
      layer._paused = true;
  ...
  describe('discard', () => {
    it('should cancel the weight promise', async () => {
      ...
      await expect(weightPromise).toResolve();
    });
