...
export class AbstractTextToSpeechFeature extends AbstractHostFeature {
  ...
  constructor(host, options = { voice: undefined, engine: undefined, language: undefined, audioFormat: 'mp3', sampleRate: undefined, speechmarkOffset: 0, minEndMarkDuration: 0.05, volume: 1, isGlobal: false }) {
    ...
    this.speechmarkOffset = Number.isNaN(Number(options.speechmarkOffset)) ? 0 : Number(options.speechmarkOffset);
    this.minEndMarkDuration = Number.isNaN(Number(options.minEndMarkDuration)) ? 0 : Number(options.minEndMarkDuration);
  ...
  /**
   * Gets the number of seconds to offset speechmark emission.
   *
   * @type {number}
   */
  ...
  /**
   * Sets the number of seconds to offset speechmark emission.
   *
   * @type {number}
   */
  ...
  /**
   * Gets the The minimum amount of time in seconds that the last speechmark of each type in a speech can have its duration property set to.
   *
   * @type number
   */
  ...
  /**
   * Sets the The minimum amount of time in seconds that the last speechmark of each type in a speech can have its duration property set to.
   *
   * @type number
   */
  ...
  _synthesizeSpeechmarks(params) {
    return this.constructor.SERVICES.polly
      ...
      .then((result) => {
        ...
        if (endMarkTypes.sentence) {
          endMarkTypes.sentence.duration = Math.max(this._minEndMarkDuration, endTime - endMarkTypes.sentence.time);
        }
        if (endMarkTypes.word) {
          endMarkTypes.word.duration = Math.max(this._minEndMarkDuration, endTime - endMarkTypes.word.time);
        }
        if (endMarkTypes.viseme) {
          endMarkTypes.viseme.duration = Math.max(this._minEndMarkDuration, endTime - endMarkTypes.viseme.time);
        }
        if (endMarkTypes.ssml) {
          endMarkTypes.ssml.duration = Math.max(this._minEndMarkDuration, endTime - endMarkTypes.ssml.time);
        }
  ...
  setVolume(volume, seconds = 0, easingFn) {
    ...
    this._promises.volume = AnimationUtils.interpolateProperty(this, 'volume', volume, { seconds, easingFn });
    ...
    this._getSpeech(text, config)
      .then((speech) => {
        ...
        if (this._currentSpeech && this._currentSpeech.playing) {
          ...
          } else if (playMethod === 'resume' && this._currentSpeech.audio !== speech.audio) {
            this._currentSpeech.cancel();
          }
        }
  ...
  pause() {
    if (this._currentSpeech && this._currentSpeech.playing) {
      ...
      console.warn(`Cannot pause speech on host ${this.host.id}. No speech is currently playing`);
    }
  }
  ...
  stop() {
    if (this._currentSpeech && this._currentSpeech.playing) {
      ...
      console.warn(`Cannot stop speech on host ${this.host.id}. No speech is currently playing.`);
    }
  }
