 ...
 * @property {any} _promises - Promises. {...this._promises, blendValue: fn}
...
export class Blend1dState extends AbstractBlendState {
  ...
  constructor(options = {}, blendStates = [], blendThresholds = [], phaseMatches = []) {
    ...
    const nonZeroDifferentSize = blendStates.length !== 0 && blendStates.length !== blendThresholds.length;
    if (nonZeroDifferentSize) {
      throw new Error(`Cannot create Blend1dState with blendThresholds ${blendThresholds}. BlendThresholds count does not match blendStates count.`);
    }

    const containsDuplicates = new Set(blendThresholds).size !== blendThresholds.length;
    if (containsDuplicates) {
      throw new Error(`Cannot create Blend1dState with blendThresholds ${blendThresholds}. BlendThresholds contains duplicate values`);
    }
  ...
  setBlendWeight(name, value, seconds = 0, easingFn) {
    ...
    this._promises.blendValue = AnimationUtils.interpolateProperty(this, 'blendValue', value, {
      seconds,
      easingFn,
      onProgress: () => {
        this._updateBlendWeights();
  ...
  addState(state, thresholdValue = 0, phaseMatch = false) {
    ...
    const sameValue = this._thresholds.find((threshold) => threshold.value === thresholdValue);
    if (this._forceNoThresholdDupes && sameValue !== undefined) {
      throw new Error(`Cannot set blend threshold of ${thresholdValue} for state ${state.name} on ${this.name}. A state already exists with that threshold.`);
    }
  ...
  removeState(name) {
    ...
    if (removed) {
      const index = this._thresholds.findIndex((threshold) => threshold.name === name);
  ...
  renameState(currentName, newName) {
    ...
    const threshold = this._thresholds.find((threshold) => threshold.name === currentName);
  ...
  getBlendThreshold(name) {
    const threshold = this._thresholds.find((threshold) => threshold.name === name);

    if (threshold === undefined) {
      throw new Error(`Cannot get blend threshold of state ${name} on ${this.name}. No state exists with that name.`);
    }
  ...
  setBlendThreshold(name, value) {
    // Make sure there is not already a state with this threshold
    const sameValue = this._thresholds.find((threshold) => threshold.value === value);
    if (sameValue !== undefined) {
      throw new Error(`Cannot set blend threshold of ${value} for state ${name} on ${this.name}. A state already exists with that threshold.`);
    }

    const threshold = this._thresholds.find((threshold) => threshold.name === name);

    if (threshold === undefined) {
      throw new Error(`Cannot set blend threshold of state ${name} on ${this.name}. No state exists with that name.`);
    }
  ...
  _updateBlendWeights() {
    ...
    if (targetIndex === 0 || targetIndex === -1) {
      ...
      const factorB = (this.blendValue - thresholdA.value) / (thresholdB.value - thresholdA.value);
