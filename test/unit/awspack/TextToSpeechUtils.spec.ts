...
describeHostEnvironment('TextToSpeechUtils', () => {
  ...
  describe('autoGenerateSSMLMarks', () => {
    ...
    it('should execute addMarksToUnmarkedSentences if there are random marks defined', () => {
      const onAddRandomMarks = jest.spyOn(TextToSpeechUtils, 'addMarksToUnmarkedSentences');
      const map = {
        'mark:no': ['no'],
        'mark:text': ['text'],
        'mark:the': ['the'],
  ...
  describe('addMarksToUnmarkedSentences', () => {
    ...
    it('should return the original input text if all sentences contain marks', () => {
      ...
      expect(TextToSpeechUtils.addMarksToUnmarkedSentences(text, ['random1', 'random2', 'random3'])).toEqual(text);
    });
    ...
    it('should add marks to each sentence not containing marks', () => {
      ...
      [
        ...
        const result = TextToSpeechUtils.addMarksToUnmarkedSentences(set[0], ['test']);
  ...
  describe('validateText', () => {
    it('should enclose the input string in <speak></speak> tags if it is not already', () => {
      expect('<speak>some text</speak>').toEqual(TextToSpeechUtils.validateText('some text'));
    ...
    it('should not add duplivate <speak></speak> tags if they are aready present', () => {
      expect('<speak>some text</speak>').toEqual(TextToSpeechUtils.validateText('<speak>some text</speak>'));
    });
    ...
    it('should remove extra spaces between speak tags and the enclosed text', () => {
      ...
      const actual = TextToSpeechUtils.validateText('<speak>  some text with weird spacing   \n  </speak>');
  ...
  describe('_processInputMap', () => {
    ...
    it('should produce a map key for each word in the value array in the input object', () => {
      const map = {'mark:no': ['no', 'this', 'test']};
  ...
  describe('_insertRandomMarksAt', () => {
    it('should return the same string if the indices array is null or zero length', () => {
      ...
      expect(TextToSpeechUtils._insertRandomMarksAt(text, null, ['mark:test'])).toEqual(text);

      expect(TextToSpeechUtils._insertRandomMarksAt(text, [], ['mark:test'])).toEqual(text);
    });

    it('should return the same string if the marks array is null or zero length', () => {
      const text = 'This is some text.';

      expect(TextToSpeechUtils._insertRandomMarksAt(text, [1, 2], null)).toEqual(text);

      expect(TextToSpeechUtils._insertRandomMarksAt(text, [1, 2], [])).toEqual(text);
    });

    it('should insert marks at the specified indices without losing any of the original text', () => {
      const text = 'This is some text';

      const result = TextToSpeechUtils._insertRandomMarksAt(text, [0, 4], ['mark:test']);
    ...
    it('should insert marks at the specified indices without losing any of the original text', () => {
      ...
      expect(result.indexOf(`<mark name='mark:test'/>`, 1)).toEqual(4 + `<mark name='mark:test'/>`.length);
    });
  });
