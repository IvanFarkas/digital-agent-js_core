...
export class HostObject extends Messenger {
  ...
  /**
   * Instantiate a new Host feature and store it. Features must inherit from AbstractHostFeature.
   *
   * @param {any} FeatureClass - Class that will instantiate the feature. Must extend {@link AbstractHostFeature}.
  ...
  addFeature(FeatureClass, force = false, ...args) {
    ...
    if (this._features[FeatureClass.name] !== undefined) {
      if (force) {
        console.warn(`Feature ${FeatureClass.name} already exists on host ${this.id}. Existing feature will be overwritten.`);
      } else {
        throw new Error(`Feature ${FeatureClass.name} already exists on host ${this.id}. Use 'force' argument to overwrite the feature.`);
      }
    }
  ...
  removeFeature(typeName) {
    if (this._features[typeName] === undefined) {
      console.warn(`Feature of type ${typeName} does not exist on host ${this.id}. No feature will be removed.`);
      return false;
    } else {
      // Remove the feature
