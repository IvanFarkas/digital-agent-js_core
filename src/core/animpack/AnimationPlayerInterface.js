  ...
  static Mixin(BaseClass = class {}) {
    const AnimationPlayerMixin = class extends BaseClass {
      constructor(options = {}, ...args) {
        ...
        this._paused = false;
        this._transitionTime = Number(options.transitionTime) >= 0 ? Number(options.transitionTime) : 0;
        this._easingFn = typeof options.easingFn === 'function' ? options.easingFn : undefined;
      }
      ...
      set transitionTime(seconds) {
        ...
        if (!(seconds >= 0)) {
          throw new Error(`Cannot set transition time for ${this.constructor.name} to ${seconds}. Seconds must be a numeric value greather than or equal to zero.`);
        }
      ...
      _prepareCurrentState(name, playMethod, transitionTime, easingFn, onError) {
        ...
        if (this.currentAnimation !== name) {
          ...
          // Blend to the new state over time
            ...
            const currentStates = [...this._states.values()].filter((s) => s !== targetState && (s.weight || s.weightPending));

            // Update the transition state with new inputs
            this._transitionState.configure(currentStates, targetState, transitionTime, easingFn, () => {
              this._currentState = targetState;
