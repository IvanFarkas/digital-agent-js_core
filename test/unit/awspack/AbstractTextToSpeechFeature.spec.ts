...
describeHostEnvironment('AbstractTextToSpeechFeature', () => {
  let mockHost: any;
  let mockPolly: any; // Polly;
  let mockPresigner: any; // Polly.Presigner;

  const mockNeuralVersion = '2.1049.0';
  const mockStandardVersion = '2.1047.0';
  const mockMarkString = '{"time":0,"type":"sentence","start":0,"end":1,"value":"t"}';
  const mockSpeechMarkResult: {AudioStream: {data: number[]}} = {AudioStream: {data: []}};
  ...
  beforeEach(() => {
    ...
    // mock AWS.Polly
    mockPolly = createSpyObj('Polly', ['synthesizeSpeech', 'describeVoices']) as unknown as Polly;
    mockPolly.synthesizeSpeech.mockReturnValue({promise: jest.fn().mockReturnValue(Promise.resolve(mockSpeechMarkResult))});
    ...
    mockPresigner = createSpyObj('Presigner', ['getSynthesizeSpeechUrl']);
    mockPresigner.getSynthesizeSpeechUrl = jest.fn((_params: any, fn: any) => fn(undefined, ''));
  });

  describe('constructor.initializeService', () => {
    it('should throw an error if any of the arguments is not defined', () => {
      expect(AbstractTextToSpeechFeature.initializeService.bind(AbstractTextToSpeechFeature, undefined, mockPresigner, mockNeuralVersion)).toThrowError();

      expect(AbstractTextToSpeechFeature.initializeService.bind(AbstractTextToSpeechFeature, mockPolly, undefined, mockNeuralVersion)).toThrowError();

      expect(AbstractTextToSpeechFeature.initializeService.bind(AbstractTextToSpeechFeature, mockPolly, mockPresigner, undefined)).toThrowError();
    });

    it('should cause constructor.isReady to return true', async () => {
      expect(AbstractTextToSpeechFeature.isReady).toBeFalse();

      await AbstractTextToSpeechFeature.initializeService(mockPolly, mockPresigner, mockNeuralVersion);

      expect(AbstractTextToSpeechFeature.isReady).toBeTrue();
    });

    it('should emit constructor.EVENTS.ready event', async () => {
      const onEmit = jest.spyOn(AbstractTextToSpeechFeature, 'emit');
    ...
    it('should set constructor.SERVICES.presigner', async () => {
      ...
      expect(AbstractTextToSpeechFeature.SERVICES.presigner).toEqual(mockPresigner);
    });

    it('should set constructor.AWS_VERSION', async () => {
      await AbstractTextToSpeechFeature.initializeService(mockPolly, mockPresigner, mockNeuralVersion);

      expect(AbstractTextToSpeechFeature.AWS_VERSION).toEqual(mockNeuralVersion);
    });
    ...
    it('should populate constructor.POLLY_LANGUAGES', async () => {
      await AbstractTextToSpeechFeature.initializeService(mockPolly, mockPresigner, mockNeuralVersion);
      const actual = Object.keys(AbstractTextToSpeechFeature.POLLY_LANGUAGES).length;
      const expected = 2;
    ...
    it('should populate constructor.POLLY_LANGUAGE_CODES', async () => {
      await AbstractTextToSpeechFeature.initializeService(mockPolly, mockPresigner, mockNeuralVersion);
      const actual = Object.keys(AbstractTextToSpeechFeature.POLLY_LANGUAGE_CODES).length;
      const expected = 2;
  ...
  describe('_validateEngine', () => {
    it('should return the original argument if it is at least 2.1048.0', async () => {
      await AbstractTextToSpeechFeature.initializeService(mockPolly, mockPresigner, mockNeuralVersion);
    ...
    it('should return "standard" if the argument is less than 2.1048.0', async () => {
      await AbstractTextToSpeechFeature.initializeService(mockPolly, mockPresigner, mockStandardVersion);
  ...
  describe('_validateRate', () => {
    ...
    it('should return the original argument if it is "8000" or "16000" when format is "pcm"', async () => {
      ...
      const tts = new AbstractTextToSpeechFeature(mockHost, {audioFormat: 'pcm'});
  ...
  describe('_validate', () => {
    // TODO: Fix P1. toHaveBeenCalledBefore
    it('should validate engine, audio format, sample rate, voice and language in order', async () => {
      await AbstractTextToSpeechFeature.initializeService(mockPolly, mockPresigner, mockStandardVersion);

      const tts = new AbstractTextToSpeechFeature(mockHost);
      const onValidateEngine = jest.spyOn(tts, '_validateEngine');
      const onValidateFormat = jest.spyOn(tts, '_validateFormat');
      const onValidateRate = jest.spyOn(tts, '_validateRate');
      const onValidateVoice = jest.spyOn(tts, '_validateVoice');
      const onValidateLanguage = jest.spyOn(tts, '_validateLanguage');
      tts._validate();
  ...
  describe('_getConfig', () => {
    it("should execute _validate if the service is initialized and the instance hasn't been validated yet", async () => {
      ...
      const tts = new AbstractTextToSpeechFeature(mockHost);
      const onValidate = jest.spyOn(tts, '_validate');
  ...
  describe('_getSpeech', () => {
    ...
    it("should be rejected if the Polly services haven't been initialized", () => {
      ...
      return expect(tts._getSpeech('some text')).toReject();
    });
    ...
    it('should be rejected if no text is defined', async () => {
      ...
      return expect(tts._getSpeech()).toReject();
    });
  ...
  describe('installApi', () => {
    it('should add "play", "pause", "resume" and "stop" methods to the feature\'s api object on the host', () => {
      ...
      expect(mockHost.AbstractTextToSpeechFeature.play).toBeInstanceOf(Function);

      expect(mockHost.AbstractTextToSpeechFeature.pause).toBeInstanceOf(Function);

      expect(mockHost.AbstractTextToSpeechFeature.resume).toBeInstanceOf(Function);

      expect(mockHost.AbstractTextToSpeechFeature.stop).toBeInstanceOf(Function);
    });
  });

  describe('volume', () => {
    let tts: AbstractTextToSpeechFeature;
  ...
  describe('pauseVolume', () => {
    ...
    it('should prevent update from executing the volume promise', () => {
      const onExecute = jest.spyOn(tts._promises.volume, 'execute');
  ...
  describe('update', () => {
    ...
    it("should execute the current speech's update function once if there is a current speech that is playing", () => {
      const mockSpeech = createSpyObj('mockSpeech', ['update'], {playing: true});
      const tts = new AbstractTextToSpeechFeature(mockHost);
  ...
  describe('play', () => {
    ...
    it('should reject the returned promise if _getSpeech was rejected because polly service has not been initialized', () => {
      ...
      return expect(tts.play('test')).toReject();
    });

    // TODO: Fix P1.
    it.skip('should reject the returned promise if _getSpeech was rejected because of bad inputs', async () => {
      await AbstractTextToSpeechFeature.initializeService(mockPolly, mockPresigner, mockNeuralVersion);

      const tts = new AbstractTextToSpeechFeature(mockHost);

      expect(await tts.play()).toReject();
    });

    // TODO: Fix P1.
    it.skip('should execute play once on the new current speech', (done) => {
      AbstractTextToSpeechFeature.initializeService(mockPolly, mockPresigner, mockNeuralVersion);

      const tts = new AbstractTextToSpeechFeature(mockHost);
      const mockSpeech = createSpyObj('mockSpeech', ['play'], {play: new Deferred((resolve: any) => resolve())});
    ...
    // TODO: Fix P1.
    it.skip('should set current speech to null once the play promise is no longer pending', async () => {
      await AbstractTextToSpeechFeature.initializeService(mockPolly, mockPresigner, mockNeuralVersion);
      ...
      await expect(tts.play('test')).toResolve();
    });
  });
  ...
  describe('pause', () => {
    it('should execute pause on the current speech once if it is playing', async () => {
      ...
      const tts = new AbstractTextToSpeechFeature(mockHost);
      const mockSpeech = createSpyObj('mockSpeech', ['pause'], {
        playing: true,
      });
  ...
  describe('resume', () => {
    ...
    it('should reject the returned promise if _getSpeech was rejected because polly service has not been initialized', () => {
      ...
      return expect(tts.resume('test')).toReject();
    });
    ...
    it.skip('should reject the returned promise if _getSpeech was rejected because of bad inputs', async () => {
      ...
      expect(await tts.resume()).toReject();
    });
    ...
    //   const tts = new AbstractTextToSpeechFeature(mockHost);
    //   const mockSpeech = createSpyObj('mockSpeech', ['cancel'], {playing: true, audio: {}});
    ...
    //   const tts = new AbstractTextToSpeechFeature(mockHost);
    //   const mockSpeech = createSpyObj('mockSpeech', {resume: new Deferred((resolve) => resolve())});
    ...
    // TODO: Fix P1.
    it.skip('should set current speech to null once the resume promise is no longer pending', async () => {
      await AbstractTextToSpeechFeature.initializeService(mockPolly, mockPresigner, mockNeuralVersion);

      const tts = new AbstractTextToSpeechFeature(mockHost);

      const mockSpeech = createSpyObj('mockSpeech', ['resume'], {resume: new Deferred((resolve: any) => resolve())});
      ...
      await expect(tts.resume('test')).toResolve();
    });
  });
  ...
  describe('stop', () => {
    it('should execute stop on the current speech once if it is playing', async () => {
      ...
      const tts = new AbstractTextToSpeechFeature(mockHost);
      const mockSpeech = createSpyObj('mockSpeech', ['stop'], {playing: true});
    ...
    it('should set the current speech to null', async () => {
      ...
      const tts = new AbstractTextToSpeechFeature(mockHost);
      const mockSpeech = createSpyObj('mockSpeech', ['stop'], {
        playing: true,
        text: 'test',
      });
