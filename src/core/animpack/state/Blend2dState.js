...
export class Blend2dState extends AbstractBlendState {
   ...
   */
  constructor(options = {}, blendStates = [], blendThresholds = [], phaseMatches = []) {
    super(options, blendStates);

    if (blendStates.length !== blendThresholds.length) {
      throw new Error(`Cannot create Blend2dState with blendStates ${blendStates} and blendThresholds ${blendThresholds}. Count of blendStates must match count of blendThresholds.`);
    }

    blendThresholds.slice(0, blendThresholds.length - 1).forEach((threshold, index) => {
      blendThresholds.slice(index + 1).forEach((otherThreshold) => {
        if (threshold[0] === otherThreshold[0] && threshold[1] === otherThreshold[1]) {
          throw new Error(`Cannot create Blend2dState with blendThresholds ${blendThresholds}. No duplicate values allowed in blendThresholds.`);
        }
      });
    });
  ...
  setBlendWeight(name, value, seconds = 0, easingFn) {
    ...
    if (property !== 'blendValueX' && property !== 'blendValueY') {
      throw new Error(`Cannot set blend weight for ${name} on ${this.name}. Blend2dState only accepts 'X' or 'Y' for setBlendWeight`);
    }

    this._promises[property].cancel();

    this._promises[property] = AnimationUtils.interpolateProperty(this, property, value, {
      seconds,
      easingFn,
      onProgress: () => {
        this._updateBlendWeights();
  ...
  getBlendWeight(name) {
    ...
    if (property !== 'X' && property !== 'Y') {
      throw new Error(`Cannot get blend weight for ${name} on ${this.name}. Blend2dState only accepts 'X' or 'Y' for getBlendWeight`);
    }
  ...
  _updateBlendWeights() {
    ...
    if (this._vertices.length === 2) {
      ...
      const triangle = this._triangles.find((triangle) => {
        return MathUtils.isPointInTriangle(this._vertices[triangle[0]], this._vertices[triangle[1]], this._vertices[triangle[2]], p);
      });
  ...
  /**
   * Sets blend weights for states corresponding to a triangle of thresholds and a given [x,y] blendValues.
   *
  ...
  _setInfluenceTriangle(triangle, p) {
    const areaA = MathUtils.triangleArea(this._vertices[triangle[1]], this._vertices[triangle[2]], p);
    const areaB = MathUtils.triangleArea(this._vertices[triangle[0]], this._vertices[triangle[2]], p);
    const areaC = MathUtils.triangleArea(this._vertices[triangle[0]], this._vertices[triangle[1]], p);
  ...
  _setInfluenceClosestPointInTriangles(p) {
    ...
    this._triangles.forEach((triangle, index) => {
      const pointA = MathUtils.closestPointOnLine(this._vertices[triangle[0]], this._vertices[triangle[1]], p);
      const pointB = MathUtils.closestPointOnLine(this._vertices[triangle[1]], this._vertices[triangle[2]], p);
      const pointC = MathUtils.closestPointOnLine(this._vertices[triangle[2]], this._vertices[triangle[0]], p);
  ...
  _setInfluenceClosestPointOnLine(p) {
    const closestPoint = MathUtils.closestPointOnLine(this._vertices[[0]], this._vertices[[1]], p);
  ...
  /**
   * Sets a lead phase state if the conditions for phase-matching are satisfied.
   *
